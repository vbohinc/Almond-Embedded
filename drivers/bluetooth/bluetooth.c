...
static bool send_cmd(const bt_cmd_t command, const char *data)
{
...
static communication_mode_t update_comm_mode(uint16_t timeout_ms)
{
	while_timeout(true, timeout_ms) {
		...
		if (fifo_strstr_pgm
		    (&in_fifo, PSTR("Time out,Fail to connect!"))) {
			clean_line();
...
bool bt_init(void (*upate_percentage) (uint16_t))
{
...
#ifdef SQUIRREL
bool bt_connect(const char *address)
{
...
bool
bt_discover(char result[8][12],
	    void (*update_callback) (const uint16_t progress))
{
	update_callback(20);
	test();
