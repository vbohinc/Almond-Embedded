...
CFLAGS += -Wa,-adhlns=$(<:%.c=$(OBJDIR)/%-$(MCU).lst)
...
CPPFLAGS += -Wa,-adhlns=$(<:%.cpp=$(OBJDIR)/%-$(MCU).lst)
...
ASFLAGS = $(ADEFS) -Wa,-adhlns=$(<:%.S=$(OBJDIR)/%-($MCU).lst),-gstabs,--listing-cont-lines=100
...
OBJ = $(SRC:%.c=$(OBJDIR)/%-$(MCU).o) $(CPPSRC:%.cpp=$(OBJDIR)/%-$(MCU).o) $(ASRC:%.S=$(OBJDIR)/%-$(MCU).o) 

# Define all listing files.
LST = $(SRC:%.c=$(OBJDIR)/%-$(MCU).lst) $(CPPSRC:%.cpp=$(OBJDIR)/%-$(MCU).lst) $(ASRC:%.S=$(OBJDIR)/%-$(MCU).lst) 
...
clean_list :
	...
	$(REMOVE) $(TARGET).sym
	$(REMOVE) $(TARGET).lss
	$(REMOVE) $(SRC:%.c=$(OBJDIR)/%-$(MCU).o)
	$(REMOVE) $(SRC:%.c=$(OBJDIR)/%-$(MCU).lst)
	$(REMOVE) $(SRC:.c=.s)
	$(REMOVE) $(SRC:.c=.d)
	$(REMOVE) $(SRC:.c=.i)
	$(REMOVEDIR) .dep
